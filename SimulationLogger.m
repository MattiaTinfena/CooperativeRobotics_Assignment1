classdef SimulationLogger < handle
    ...
    methods
        ...
        function plotAll(obj)
            ...
            grid on; set(gca, 'FontSize', FS); xlim([0, T_MAX]);
            subplot(2,1,2);
            ...
            plot(t_plot, obj.v_lin_tool_l(1:3, 1:obj.curr_loop), '-', 'LineWidth', LW);
            plot(t_plot, obj.v_lin_obj_l(1:3, 1:obj.curr_loop), '--', 'LineWidth', LW);
            ...
            ylabel('Linear Vel [m/s]', 'FontSize', FS); title('Linear velocities', 'FontSize', FS_Title);
            legend('Tx','Ty','Tz','Ox','Oy','Oz', 'Location', 'eastoutside'); xlim([0, T_MAX]);
            subplot(2,1,2); hold on;
            plot(t_plot, obj.v_ang_tool_l(1:3, 1:obj.curr_loop), '-', 'LineWidth', LW);
            plot(t_plot, obj.v_ang_obj_l(1:3, 1:obj.curr_loop), '--', 'LineWidth', LW);
            for s = 1:length(obj.switch_times)
                xline(obj.switch_times(s), '--k', 'LineWidth', LW_GRID, 'Alpha', 0.6, 'HandleVisibility', 'off');
            end
            hold off; grid on; set(gca, 'FontSize', FS);
            ylabel('Angular Vel [rad/s]', 'FontSize', FS); xlabel('Time [s]', 'FontSize', FS);
            ...
            subplot(2,1,1); hold on;
            plot(t_plot, obj.v_lin_tool_r(1:3, 1:obj.curr_loop), '-', 'LineWidth', LW);
            plot(t_plot, obj.v_lin_obj_r(1:3, 1:obj.curr_loop), '--', 'LineWidth', LW);
            for s = 1:length(obj.switch_times)
                xline(obj.switch_times(s), '--k', 'LineWidth', LW_GRID, 'Alpha', 0.6, 'HandleVisibility', 'off');
            end
            hold off; grid on; set(gca, 'FontSize', FS);
            ...
            legend('Tx','Ty','Tz','Ox','Oy','Oz', 'Location', 'eastoutside'); xlim([0, T_MAX]);
            subplot(2,1,2); hold on;
            plot(t_plot, obj.v_ang_tool_r(1:3, 1:obj.curr_loop), '-', 'LineWidth', LW);
            plot(t_plot, obj.v_ang_obj_r(1:3, 1:obj.curr_loop), '--', 'LineWidth', LW);
            for s = 1:length(obj.switch_times)
                xline(obj.switch_times(s), '--k', 'LineWidth', LW_GRID, 'Alpha', 0.6, 'HandleVisibility', 'off');
            end
            hold off; grid on; set(gca, 'FontSize', FS);
            ...
            hold off; grid on; xlabel('Time [s]', 'FontSize', FS); ylabel('Activation (A * ap)', 'FontSize', FS);
            title('Global Task Activations Timeline', 'FontSize', FS_Title); set(gca, 'FontSize', FS);
            ...
            for k = 1:obj.total_tasks
                ...
                q_L_data = obj.ql(:, 1:obj.curr_loop); q_R_data = obj.qr(:, 1:obj.curr_loop);
                ...
                data_xdot = obj.fillWithZeros(raw_xdot); data_A = obj.fillWithZeros(raw_A);
                data_ap = obj.fillWithZeros(raw_ap);
                f = figure(100 + k); clf; f.Name = sprintf("Task Analysis: %s", task_name_str);
                tg = uitabgroup(f);
                for m = 1:length(metrics_config)
                    conf = metrics_config{m}; data_L = conf{1}; data_R = conf{2}; y_lab = conf{3}; t_tit = conf{4};
                    ...
                    plot(ax2, t_plot, data_ap', '-b', 'LineWidth', LW); plot(ax2, t_plot, data_A', '--k', 'LineWidth', LW);
                    for s = 1:length(obj.switch_times), xline(ax2, obj.switch_times(s), ':k', 'Alpha', 0.5, 'LineWidth', LW_GRID, 'HandleVisibility', 'off'); end
                    hold(ax2, 'off'); grid(ax2, 'on'); set(ax2, 'FontSize', FS); ylabel(ax2, 'Activations', 'FontSize', FS); xlim(ax2, [0, T_MAX]);
                    ax3 = subplot(3,1,3, 'Parent', tab); hold(ax3, 'on');
                    if m == 1
                        colors = lines(7);
                        if ~isempty(data_L), for j=1:size(data_L,1), plot(ax3,t_plot,data_L(j,:),'Color',colors(j,:),'LineWidth',LW); end, end
                        if ~isempty(data_R), for j=1:size(data_R,1), plot(ax3,t_plot,data_R(j,:),'Color',colors(mod(j-1,7)+1,:),'LineStyle','--','LineWidth',LW); end, end
                    else
                        if ~isempty(data_L), plot(ax3,t_plot,data_L','b','LineWidth',LW); end
                        if ~isempty(data_R), plot(ax3,t_plot,data_R','r--','LineWidth',LW); end
                    end
        ...
        function data_mat = fillWithZeros(~, cell_data)
            ...
            for k = 1:length(cell_data)
                val = cell_data{k};
                if ~isempty(val) && ~isscalar(val), dim = size(val, 1); break; end
            end
            for k = 1:length(cell_data)
                if isempty(cell_data{k}) || (isscalar(cell_data{k}) && dim > 1), cell_data{k} = zeros(dim, 1); end
            end
            data_mat = cell2mat(cell_data');
            if size(data_mat, 2) ~= length(cell_data), data_mat = data_mat'; end
        end
    end
end